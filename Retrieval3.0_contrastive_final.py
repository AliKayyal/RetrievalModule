...
class NeuralNetwork_test(nn.Module):
    ...
    def forward(self, Q, embedded_abstract):
        ...
        embedded_abstract = torch.tensor(embedded_abstract)
        embedded_abstract = torch.reshape(embedded_abstract, [5, 768])
...
def test_with_label_and_ambeddings (quries, abstracts,embeddings, label, model, device, batch_size=4):
    size = len(label)
    model.eval()
...
def find_embeddings(PMID):#find the embedded abstract using the PMID of the original abstract
    with open("PMID_abstracts_embeddings.json", 'r') as json_file:  # training data
    ...
    for i in range(size):
        for j in range(size):
            if PMID[i] == PMID_abstracts_embeddings[j][0]:
                embeddings.append((PMID_abstracts_embeddings[j][2]))
                break
...
def test_model_with_embeddings():
    ...
    # load model
    model.load_state_dict(torch.load("Retrieval_model_contrastive.pth", map_location=torch.device('cpu')),strict = False)
    #model.load_state_dict(torch.load("Retrieval_model_contrastive.pth"))

    # test model on testing data
    pred_label, selected_abstracts = test_with_label_and_ambeddings(queries,abstracts, test_embeddings, test_label, model, device, batch_size)
...
def retrieval(test_data):
    ...
    # load model
    model.load_state_dict(torch.load("Retrieval_model_contrastive.pth", map_location=torch.device('cpu')))
